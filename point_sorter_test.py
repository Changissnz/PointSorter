...
class TestPointSorterClass(unittest.TestCase):
    ...
    def test_sorted_vector_value_search(self):
        ...
        assert sorted_vector_value_search(tv1, 6.7)[1]
        assert sorted_vector_value_search(tv2, 6.7)[1]
        assert sorted_vector_value_search(tv3, 6.7)[1]
        assert not sorted_vector_value_search(tv4, 6.7)[1]

    def test_sorted_vector_value_range_search(self):
        q = sample_pointsort_vector_1()
        vec = q[:,0]
        vec = np.sort(list(vec) + [3.1])
        q = sorted_vector_value_range_search(vec, 3.1)
        assert q[0] == [4,6], "invalid range search #1"

        vec2 = np.array([6.6,6.6])
        q = sorted_vector_value_range_search(vec2, 6.6)
        assert q[0] == [0,1], "invalid range search #2"

        vec3 = np.array([0.2, 0.4, 0.4, 0.4, 3.1, 6.71, 9.1, 20.1])
        q = sorted_vector_value_range_search(vec3, 0.4)
        assert q[0] == [1,3]

        vec4 = np.array([0.2, 0.4, 3.1, 6.71, 9.1, 20.1])
        q = sorted_vector_value_range_search(vec4, 0.4)
        assert q[0] == [1,1]
    ...
    def test__PointSorter__update_points_from_cache():
        q = sample_pointsort_vector_1()
        np.random.shuffle(q)
        q2 = np.empty((0,q.shape[1]))
        ps = PointSorter(q2)
        ...
        assert equal_iterables(ps.extremumData[-1][0],np.array([5.0,5.0,5.0]))
        assert equal_iterables(ps.extremumData[1][0],[   2.,   10., 1000.])
        ...
        assert equal_iterables(ps.extremumData[-1][0],np.array([5.0,5.0,5.0]))
        assert equal_iterables(ps.extremumData[1][0],[ 1., 15., 31.])
